...
class supplierclass:
    def __init__(self, root):
        ...
        Entry(self.root, textvariable=self.var_searctxt, font=("goudy old style", 15),
        ...
        # =======title======
        title = Label(self.root, text="Supplier Details", font=("goudy old style", 20,"bold"), bg="#0f4d7d", fg="white").place(
            x=50, y=10, width=1000,height=40)
        ...
        self.txt_desc.place(x=180, y=200, width=470, height=120)  # Corrected this line
                ...
                if row is not None:
                    ...
                    cur.execute(
                        ...
                        (
                            ...
                            self.txt_desc.get('1.0', END).strip(),  # Use .strip() to remove trailing newline
                        ))
                    con.commit()
    ...
    def search(self):
        ...
        try:
            ...
            if self.var_searctxt.get() == "":
                ...
                cur.execute(
                    "SELECT * FROM supplier where invoice=?", (self.var_searctxt.get(),))
                row = cur.fetchone()
                if row != None:
                    self.supplierTable.delete(*self.supplierTable.get_children())
                    self.supplierTable.insert('', END, values=row)
